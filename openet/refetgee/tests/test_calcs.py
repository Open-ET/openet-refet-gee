...
def test_air_pressure_default(elev=s_args['elev'], expected=s_args['pair_asce']):
    output = utils.get_info(calcs._air_pressure(elev=ee.Number(elev)))
    assert float(output) == pytest.approx(expected)

def test_air_pressure_asce(elev=s_args['elev'], expected=s_args['pair_asce']):
    output = utils.get_info(calcs._air_pressure(elev=ee.Number(elev), method='asce'))
    assert float(output) == pytest.approx(expected)

def test_air_pressure_refet(elev=s_args['elev'], expected=s_args['pair']):
    output = utils.get_info(calcs._air_pressure(elev=ee.Number(elev), method='refet'))
    assert float(output) == pytest.approx(expected)

def test_air_pressure_position(elev=s_args['elev'], expected=s_args['pair']):
    output = utils.get_info(calcs._air_pressure(ee.Number(elev), 'refet'))
    assert float(output) == pytest.approx(expected)

def test_air_pressure_image(elev=s_args['elev'], expected=s_args['pair_asce']):
    output = utils.constant_image_value(calcs._air_pressure(elev=ee.Image.constant(elev)))
...
def test_sat_vapor_pressure_number(tdew, expected):
    output = utils.get_info(calcs._sat_vapor_pressure(ee.Number(tdew)))
    assert float(output) == pytest.approx(expected)

def test_sat_vapor_pressure_keyword(tdew=d_args['tdew'], expected=d_args['ea']):
    output = utils.get_info(calcs._sat_vapor_pressure(temperature=ee.Number(tdew)))
    assert float(output) == pytest.approx(expected)

def test_sat_vapor_pressure_image(tdew=d_args['tdew'], expected=d_args['ea']):
    output = utils.constant_image_value(calcs._sat_vapor_pressure(ee.Image.constant(tdew)))
    assert float(output['constant']) == pytest.approx(expected)
...
def test_actual_vapor_pressure_number(q=d_args['q'], pair=s_args['pair'], expected=d_args['ea']):
    output = utils.get_info(calcs._actual_vapor_pressure(q=ee.Number(q), pair=ee.Number(pair)))
    assert float(output) == pytest.approx(expected)
...
def test_actual_vapor_pressure_position(q=d_args['q'], pair=s_args['pair'], expected=d_args['ea']):
    output = utils.get_info(calcs._actual_vapor_pressure(ee.Number(q), ee.Number(pair)))
    assert float(output) == pytest.approx(expected)

def test_actual_vapor_pressure_image(q=d_args['q'], pair=s_args['pair'], ea=d_args['ea']):
    output = utils.constant_image_value(calcs._actual_vapor_pressure(
...
def test_vpd_number(es=d_args['es'], ea=d_args['ea']):
    output = utils.get_info(calcs._vpd(es=ee.Number(es), ea=ee.Number(ea)))
    assert float(output) == pytest.approx(float(es - ea))

    # Check that negative VPD's are set to 0
    output = utils.get_info(calcs._vpd(es=ee.Number(es), ea=ee.Number(es + 1)))
    assert float(output) == pytest.approx(0)

def test_vpd_image(es=d_args['es'], ea=d_args['ea']):
    output = utils.constant_image_value(calcs._vpd(es=ee.Image.constant(es), ea=ee.Number(ea)))
...
def test_es_slope_default(tmin=d_args['tmin'], tmax=d_args['tmax'], expected=d_args['es_slope_asce']):
    output = utils.get_info(calcs._es_slope(tmean=ee.Number(0.5 * (tmin + tmax))))
    assert float(output) == pytest.approx(expected)

def test_es_slope_asce(tmin=d_args['tmin'], tmax=d_args['tmax'], expected=d_args['es_slope_asce']):
    output = utils.get_info(calcs._es_slope(
...
def test_es_slope_refet(tmin=d_args['tmin'], tmax=d_args['tmax'], expected=d_args['es_slope']):
    output = utils.get_info(calcs._es_slope(tmean=ee.Number(0.5 * (tmin + tmax)), method='refet'))
    assert float(output) == pytest.approx(expected)

def test_es_slope_position(tmin=d_args['tmin'], tmax=d_args['tmax'], expected=d_args['es_slope']):
    output = utils.get_info(calcs._es_slope(ee.Number(0.5 * (tmin + tmax)), 'refet'))
    assert float(output) == pytest.approx(expected)

def test_es_slope_image(tmin=d_args['tmin'], tmax=d_args['tmax'], expected=d_args['es_slope_asce']):
    output = utils.constant_image_value(calcs._es_slope(
...
def test_precipitable_water_number(ea=d_args['ea'], pair=s_args['pair'], expected=d_args['w']):
    output = utils.get_info(calcs._precipitable_water(ee.Number(ea), ee.Number(pair)))
    assert float(output) == pytest.approx(expected)

def test_precipitable_water_image_1(ea=d_args['ea'], pair=s_args['pair'], expected=d_args['w']):
    output = utils.constant_image_value(calcs._precipitable_water(
...
def test_doy_fraction_number(doy=d_args['doy'], expected=d_args['doy_frac']):
    assert float(utils.get_info(calcs._doy_fraction(ee.Number(doy)))) == pytest.approx(expected)

def test_doy_fraction_keyword(doy=d_args['doy'], expected=d_args['doy_frac']):
    output = utils.get_info(calcs._doy_fraction(doy=ee.Number(doy)))
    assert float(output) == pytest.approx(expected)

def test_doy_fraction_image(doy=d_args['doy'], expected=d_args['doy_frac']):
    output = utils.constant_image_value(calcs._doy_fraction(ee.Image.constant(doy)))
    assert float(output['constant']) == pytest.approx(expected)


def test_delta_default(doy=d_args['doy'], expected=d_args['delta_asce']):
    assert float(utils.get_info(calcs._delta(ee.Number(doy)))) == pytest.approx(expected)

def test_delta_asce(doy=d_args['doy'], expected=d_args['delta_asce']):
    output = utils.get_info(calcs._delta(ee.Number(doy), method='asce'))
    assert float(output) == pytest.approx(expected)

def test_delta_refet(doy=d_args['doy'], expected=d_args['delta']):
    output = utils.get_info(calcs._delta(ee.Number(doy), method='refet'))
    assert float(output) == pytest.approx(expected)

def test_delta_keyword(doy=d_args['doy'], expected=d_args['delta_asce']):
    assert float(utils.get_info(calcs._delta(doy=ee.Number(doy)))) == pytest.approx(expected)

def test_delta_image(doy=d_args['doy'], expected=d_args['delta_asce']):
    output = utils.constant_image_value(calcs._delta(ee.Image.constant(doy)))
    assert float(output['constant']) == pytest.approx(expected)


def test_dr_number(doy=d_args['doy'], expected=d_args['dr']):
    assert float(utils.get_info(calcs._dr(ee.Number(doy)))) == pytest.approx(expected)

def test_dr_keyword(doy=d_args['doy'], expected=d_args['dr']):
    assert float(utils.get_info(calcs._dr(doy=ee.Number(doy)))) == pytest.approx(expected)

def test_dr_image(doy=d_args['doy'], expected=d_args['dr']):
    output = utils.constant_image_value(calcs._dr(ee.Image.constant(doy)))
    assert float(output['constant']) == pytest.approx(expected)


def test_seasonal_correction_number(doy=d_args['doy'], expected=d_args['sc']):
    output = utils.get_info(calcs._seasonal_correction(ee.Number(doy)))
    assert float(output) == pytest.approx(expected)

def test_seasonal_correction_keyword(doy=d_args['doy'], expected=d_args['sc']):
    output = utils.get_info(calcs._seasonal_correction(doy=ee.Number(doy)))
    assert float(output) == pytest.approx(expected)

def test_seasonal_correction_image(doy=d_args['doy'], expected=d_args['sc']):
    output = utils.constant_image_value(calcs._seasonal_correction(ee.Image.constant(doy)))
    assert float(output['constant']) == pytest.approx(expected)
...
def test_solar_time_rad_number(
    ...
    output = utils.get_info(calcs._solar_time_rad(
...
def test_solar_time_rad_image(
    ...
    output = utils.constant_image_value(calcs._solar_time_rad(
...
def test_omega_number(solar_time=h_args['solar_time'], expected=h_args['omega']):
    output = utils.get_info(calcs._omega(ee.Number(solar_time)))
    assert float(output) == pytest.approx(expected)

def test_omega_keyword(solar_time=h_args['solar_time'], expected=h_args['omega']):
    output = utils.get_info(calcs._omega(solar_time=ee.Number(solar_time)))
    assert float(output) == pytest.approx(expected)

def test_omega_image(solar_time=h_args['solar_time'], expected=h_args['omega']):
    output = utils.constant_image_value(calcs._omega(ee.Image.constant(solar_time)))
    assert float(output['constant']) == pytest.approx(expected)
...
def test_wrap(x, x_min, x_max, expected):
    assert float(utils.get_info(calcs._wrap(ee.Number(x), x_min, x_max))) == pytest.approx(expected)


def test_omega_sunset_number(lat=s_args['lat'], delta=d_args['delta'], expected=d_args['omega_s']):
    output = utils.get_info(calcs._omega_sunset(lat=ee.Number(lat), delta=ee.Number(delta)))
    assert float(output) == pytest.approx(expected)

def test_omega_sunset_position(lat=s_args['lat'], delta=d_args['delta'], expected=d_args['omega_s']):
    output = utils.get_info(calcs._omega_sunset(ee.Number(lat), ee.Number(delta)))
    assert float(output) == pytest.approx(expected)

def test_omega_sunset_image(lat=s_args['lat'], delta=d_args['delta'], expected=d_args['omega_s']):
    output = utils.constant_image_value(calcs._omega_sunset(
...
def test_ra_daily_default(lat=s_args['lat'], doy=d_args['doy'], expected=d_args['ra_asce']):
    output = utils.get_info(calcs._ra_daily(lat=ee.Number(lat), doy=ee.Number(doy)))
    assert float(output) == pytest.approx(expected)

def test_ra_daily_asce(lat=s_args['lat'], doy=d_args['doy'], expected=d_args['ra_asce']):
    output = utils.get_info(calcs._ra_daily(
...
def test_ra_daily_position(lat=s_args['lat'], doy=d_args['doy'], expected=d_args['ra_asce']):
    output = utils.get_info(calcs._ra_daily(ee.Number(lat), ee.Number(doy)))
    assert float(output) == pytest.approx(expected)

def test_ra_daily_image(lat=s_args['lat'], doy=d_args['doy'], expected=d_args['ra_asce']):
    output = utils.constant_image_value(calcs._ra_daily(
...
#                          doy=h_args['doy'], time_mid=h_args['time_mid'],
#                          expected=h_args['ra_asce']):
#     output = utils.constant_image_value(calcs._ra_hourly(
#         lat=ee.Number(lat), lon=ee.Number(lon), doy=ee.Number(doy),
...
def test_rso_daily_number(
    ...
    output = utils.get_info(calcs._rso_daily(
...
def test_rso_daily_image_doy_number(
    ...
    output = utils.constant_image_value(calcs._rso_daily(
...
def test_rso_hourly_default(ea=h_args['ea'], ra=h_args['ra'],
    ...
    output = utils.get_info(calcs._rso_hourly(
...
def test_rso_hourly_image(ea=h_args['ea'], ra=h_args['ra'],
                          ...
                          lat=s_args['lat'], lon=s_args['lon'],
                          expected=h_args['rso_asce']):
    output = utils.constant_image_value(calcs._rso_hourly(
...
def test_rso_simple_number(ra, elev, expected):
    output = utils.get_info(calcs._rso_simple(ra=ee.Number(ra), elev=ee.Number(elev)))
    assert float(output) == pytest.approx(expected)

def test_rso_simple_position(ra=d_args['ra'], elev=s_args['elev'], expected=d_args['rso_simple']):
    output = utils.get_info(calcs._rso_simple(ee.Number(ra), ee.Number(elev)))
    assert float(output) == pytest.approx(expected)

def test_rso_simple_image(ra=d_args['ra'], elev=s_args['elev'], expected=d_args['rso_simple']):
    output = utils.constant_image_value(calcs._rso_simple(
...
def test_fcd_daily_number(rs=d_args['rs'], rso=d_args['rso'], expected=d_args['fcd']):
    output = utils.get_info(calcs._fcd_daily(rs=ee.Number(rs), rso=ee.Number(rso)))
    assert float(output) == pytest.approx(expected)

def test_fcd_daily_position(rs=d_args['rs'], rso=d_args['rso'], expected=d_args['fcd']):
    output = utils.get_info(calcs._fcd_daily(ee.Number(rs), ee.Number(rso)))
    assert float(output) == pytest.approx(expected)

def test_fcd_daily_image(rs=d_args['rs'], rso=d_args['rso'], expected=d_args['fcd']):
    output = utils.constant_image_value(calcs._fcd_daily(
...
def test_fcd_hourly_image(rs=h_args['rs'], rso=h_args['rso'],
                          ...
                          lat=s_args['lat'], lon=s_args['lon'],
                          expected=h_args['fcd_asce']):
    output = utils.constant_image_value(calcs._fcd_hourly(
...
def test_rnl_hourly_number(tmean=h_args['tmean'], ea=h_args['ea'],
    ...
    output = utils.get_info(calcs._rnl_hourly(
...
def test_rnl_hourly_image(tmean=h_args['tmean'], ea=h_args['ea'],
                          fcd=h_args['fcd'], expected=h_args['rnl']):
    output = utils.constant_image_value(calcs._rnl_hourly(
...
def test_rn_number(rs=d_args['rs'], rnl=d_args['rnl'], expected=d_args['rn']):
    output = utils.get_info(calcs._rn(rs=ee.Number(rs), rnl=ee.Number(rnl)))
    assert float(output) == pytest.approx(expected)

def test_rn_keywods(rs=d_args['rs'], rnl=d_args['rnl'], expected=d_args['rn']):
    output = utils.get_info(calcs._rn(ee.Number(rs), ee.Number(rnl)))
    assert float(output) == pytest.approx(expected)

def test_rn_image(rs=d_args['rs'], rnl=d_args['rnl'], expected=d_args['rn']):
    output = utils.constant_image_value(calcs._rn(
...
def test_wind_height_adjust_2m(uz=2.5, zw=2.0, expected=2.5):
    output = utils.get_info(calcs._wind_height_adjust(uz=ee.Number(uz), zw=ee.Number(zw)))
...
def test_wind_height_adjust_position(uz=d_args['uz'], zw=s_args['zw'], expected=d_args['u2']):
    output = utils.get_info(calcs._wind_height_adjust(ee.Number(uz), ee.Number(zw)))
    assert float(output) == pytest.approx(expected)
...
def test_wind_height_adjust_image(uz, zw, expected):
    output = utils.constant_image_value(calcs._wind_height_adjust(
