...
def test_air_pressure_keyword(elev=s_args['elev'], pair=s_args['pair_asce']):
    assert float(calcs._air_pressure(
...
# def test_sat_vapor_pressure_image(tdew=d_args['ea'], ea=d_args['ea']):
...
def test_specific_humidity_daily_image(tdew=d_args['tdew'], ea=d_args['ea']):
    output = calcs._sat_vapor_pressure(ee.Image.constant(tdew)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
    assert float(output['constant']) == pytest.approx(ea)
...
def test_specific_humidity_number(ea=d_args['ea'], pair=s_args['pair'],
                                  q=d_args['q']):
    assert float(calcs._specific_humidity(
        ee.Number(ea), ee.Number(pair)).getInfo()) == pytest.approx(q)

def test_specific_humidity_keyword(ea=d_args['ea'], pair=s_args['pair'],
                                   q=d_args['q']):
    assert float(calcs._specific_humidity(
...
def test_actual_vapor_pressure_keyword(q=d_args['q'], pair=s_args['pair'],
                                       ea=d_args['ea']):
    assert float(calcs._actual_vapor_pressure(
...
def test_actual_vapor_pressure_image(q=d_args['q'], pair=s_args['pair_asce'],
    ...
    output = calcs._actual_vapor_pressure(
            ee.Image.constant(q), ee.Number(pair))\
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1)\
        .getInfo()
    assert float(output['constant']) == pytest.approx(ea)
...
def test_vpd_image(es=d_args['es'], ea=d_args['ea']):
    output = calcs._vpd(ee.Image.constant(es), ee.Number(ea))\
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
...
def test_es_slope_keyword(tmin=d_args['tmin'], tmax=d_args['tmax'],
                          es_slope=d_asce_args['es_slope']):
    assert float(calcs._es_slope(
...
def test_delta_keyword(doy=d_args['doy'], delta=d_asce_args['delta']):
    assert float(calcs._delta(
        doy=ee.Number(doy)).getInfo()) == pytest.approx(delta)

def test_delta_image(doy=d_args['doy'], delta=d_asce_args['delta']):
...
def test_omega_sunset_keyword(lat=s_args['lat'], delta=d_args['delta'],
                              omega_s=d_args['omega_s']):
    assert float(calcs._omega_sunset(
        lat=ee.Number(lat),
...
def test_ra_daily_keyword(lat=s_args['lat'], doy=d_args['doy'],
                          ra=d_asce_args['ra']):
    assert float(calcs._ra_daily(
...
def test_ra_hourly_refet(lat=s_args['lat'], lon=s_args['lon'],
    ...
    assert float(calcs._ra_hourly(
        ee.Number(lat), ee.Number(lon), ee.Number(doy),
        ee.Number(time_mid), method='refet').getInfo()) == pytest.approx(ra)

def test_ra_hourly_keyword(lat=s_args['lat'], lon=s_args['lon'],
                           doy=h_args['doy'], time_mid=h_args['time_mid'],
                           ra=h_asce_args['ra']):
    assert float(calcs._ra_hourly(
...
def test_rso_daily_keyword(ea=d_args['ea'], ra=d_args['ra'],
                           pair=s_args['pair'], doy=d_args['doy'],
                           lat=s_args['lat'], rso=d_args['rso']):
    assert float(calcs._rso_daily(
...
def test_rso_daily_image(ea=d_args['ea'], ra=d_args['ra'],
    ...
    output = calcs._rso_daily(
            ee.Image.constant(ea), ee.Number(ra), ee.Number(pair),
...
def test_rso_hourly_default(ea=h_args['ea'], ra=h_args['ra'],
    ...
    assert float(calcs._rso_hourly(
        ...
        ee.Number(doy), ee.Number(time_mid), ee.Number(lat),
...
def test_rso_hourly_asce(ea=h_args['ea'], ra=h_args['ra'], pair=s_args['pair'],
    ...
    assert float(calcs._rso_hourly(
        ee.Number(ea), ee.Number(ra), ee.Number(pair),
        ee.Number(doy), ee.Number(time_mid), ee.Number(lat), ee.Number(lon),
...
def test_rso_hourly_refet(ea=h_args['ea'], ra=h_args['ra'],
    ...
    assert float(calcs._rso_hourly(
        ee.Number(ea), ee.Number(ra), ee.Number(pair),
        ee.Number(doy), ee.Number(time_mid),
        ee.Number(lat), ee.Number(lon),
        method='refet').getInfo()) == pytest.approx(rso)

def test_rso_hourly_keyword(ea=h_args['ea'], ra=h_args['ra'],
                            pair=s_args['pair'], doy=h_args['doy'],
                            time_mid=h_args['time_mid'], lat=s_args['lat'],
                            lon=s_args['lon'], rso=h_asce_args['rso']):
    assert float(calcs._rso_hourly(
...
def test_rso_hourly_image(ea=h_args['ea'], ra=h_args['ra'],
    ...
    output = calcs._rso_hourly(
            ee.Image.constant(ea), ee.Image.constant(ra), ee.Number(pair),
            ee.Number(doy), ee.Number(time_mid), ee.Number(lat), ee.Number(lon)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
    assert float(output['constant']) == pytest.approx(rso)
...
def test_rso_simple_keyword(ra=d_args['ra'], elev=s_args['elev'],
...
def test_rso_simple_image(ra=d_args['ra'], elev=s_args['elev'],
                          rso=d_args['rso_simple']):
    output = calcs._rso_simple(ee.Image.constant(ra), ee.Number(elev)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
    assert float(output['constant']) == pytest.approx(rso)
...
def test_fcd_daily_keyword(rs=d_args['rs'], rso=d_args['rso'], fcd=d_args['fcd']):
    assert float(calcs._fcd_daily(
...
def test_fcd_daily_image(rs=d_args['rs'], rso=d_args['rso'], fcd=d_args['fcd']):
    output = calcs._fcd_daily(ee.Image.constant(rs), ee.Number(rso)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
...
def test_fcd_hourly_keyword(rs=h_args['rs'], rso=h_args['rso'],
                            doy=h_args['doy'], time_mid=h_args['time_mid'],
                            lat=s_args['lat'], lon=s_args['lon'],
                            fcd=h_asce_args['fcd']):
    assert float(calcs._fcd_hourly(
...
def test_fcd_hourly_image(rs=h_args['rs'], rso=h_args['rso'],
    ...
    output = calcs._fcd_hourly(
            ee.Image.constant(rs), ee.Number(rso), ee.Number(doy),
...
def test_rnl_daily_keywods(tmin=d_args['tmin'], tmax=d_args['tmax'],
...
def test_rnl_daily_image(tmin=d_args['tmin'], tmax=d_args['tmax'],
    ...
    output = calcs._rnl_daily(
            ee.Image.constant(tmax), ee.Number(tmin),
            ee.Number(ea), ee.Number(fcd)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
...
def test_rnl_hourly_keyword(tmean=h_args['tmean'], ea=h_args['ea'],
                            fcd=h_args['fcd'], rnl=h_args['rnl']):
    assert float(calcs._rnl_hourly(
...
def test_rnl_hourly_image(tmean=h_args['tmean'], ea=h_args['ea'],
    ...
    output = calcs._rnl_hourly(
            ee.Image.constant(tmean), ee.Number(ea), ee.Number(fcd)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
    assert float(output['constant']) == pytest.approx(rnl)
...
def test_rn_image(rs=d_args['rs'], rnl=d_args['rnl'], rn=d_args['rn']):
    output = calcs._rn(ee.Number(rs), ee.Image.constant(rnl)) \
        .reduceRegion(ee.Reducer.first(), geometry=constant_geom, scale=1) \
        .getInfo()
...
def test_wind_height_adjust_keyword(uz=d_args['uz'], zw=s_args['zw'],
                                    u2=d_args['u2']):
    assert float(calcs._wind_height_adjust(
